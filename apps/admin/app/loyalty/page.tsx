      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 16, margin: '12px 0 24px' }}>
        {gifts.map((gift) => (
          <div key={gift.id} style={{ ...panelStyle, padding: 18 }}>
            ...
            <strong style={{ display: 'block', fontSize: 16, margin: '12px 0 4px' }}>{gift.name}</strong>
            <p style={{ margin: 0, color: colors.brandOrange, fontWeight: 900, fontSize: 18 }}>{gift.points.toLocaleString('vi-VN')} điểm</p>
            <p style={{ margin: '6px 0 0', color: colors.brandGrey, fontSize: 13 }}>Tồn kho: {gift.stock ?? 0}</p>
          </div>
        ))}
      </div>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {gifts.map((gift) => (
                <tr key={gift.id}>
                  <td style={{ ...tableCellStyle, fontWeight: 800 }}>{gift.name}</td>
